...
func _process(delta):
	...
	if fireWormPlayer.get_current_animation() == "Walk Right":
		# calcular la velocidad basada en la variable de velocidad
		velocity.x += speed * delta
		direction = "Right"
	elif fireWormPlayer.get_current_animation() == "Walk Left":
		# calcular la velocidad basada en la variable de velocidad
		velocity.x -= speed * delta
		direction = "Left"
...
func _on_Area2D_area_exited(enemyHitBox):
	if (enemyHitBox.kinematic.direction == "Left" and direction == "Right") or (enemyHitBox.kinematic.direction == "Right" and direction == "Left"):
		if currentEnemyHitBox == enemyHitBox:
			...
			attacking = false
			fireWormPlayer.play("Walk " + direction)
		if wasBehind:
			attacking = false
			fireWormPlayer.play("Walk " + direction)
...
func _on_FireWormPlayer_animation_finished(anim_name):
	if anim_name == "Attack Right":
		...
		fireWormPlayer.play("Attack Left")
		attacking = false
